...
<script>
document.getElementById('display').value = $(display);  //50개씩 보기 누르면 50이 option으로 떠있는 모양.
document.getElementById('display').addEventListener('change', (evt)=>{
	location.href = '${contextPath}/board/list.brd?page=1&sort=${sort}&display=' + evt.target.value;  //sort와 display를 추가해서 매개변수 풀리지 않고 유지될 수 있도록 함.
})
