...
public class DataBind3 extends HttpServlet {
     ...
     */
	  ...
	  // HttpServletRequest는 redirect 로 이동할 수 없다. forward로 이동해야함. 
	  
	  // + request 가 중요. 웹에서 대부분 정보는 request에서 저장되어 활용됨.
	  //request.setAttribute("c", "일반데이터"); -> request.getRequestDispatcher("/dataConfirm").forward(request, response);
